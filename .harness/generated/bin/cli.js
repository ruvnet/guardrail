...
// Generated by metaharness — the `guardrail-agent` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx guardrail-agent`
...
// "bin" + "files" fields in package.json), so `npx guardrail-agent` works the moment
...
const HARNESS_NAME = 'guardrail-agent';

/** `guardrail-agent init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `guardrail-agent doctor` — verify the install end-to-end (kernel + host resolve). */
